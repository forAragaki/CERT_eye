...
class Cert_notAfter extends React.Component{
  static propTypes = {
  ...
  handledata(){
    const url = "https://nanase.ink:5001/cert_notAfter"+"?school="+ global.constants.param;
    ...
    backgroundColor.push(temp_backgroundColor[0]);
    backgroundColor.push(temp_backgroundColor[1]);
    for (var k in datas){
      connection.push(k);
      number.push(datas[k]);
    //   backgroundColor.push(temp_backgroundColor[Math.floor(Math.random()*5)]);
    }
...
export default function(props){
  ...
  return <Cert_notAfter {...props} theme={theme} classes={classes}/>;
}